...
mod weights;
pub mod xcm_config;
use crate::xcm_config::CurrencyHooks;
...
pub struct AssetLocation(pub xcm::v1::MultiLocation);

impl Default for AssetLocation {
	fn default() -> Self {
		AssetLocation(xcm::v2::MultiLocation::here())
	}
}
...
parameter_types! {
	...
	pub const SequentailIdStartAt: u32 = 1_000_000;
}
...
impl pallet_asset_registry::Config for Runtime {
	...
	type SequentialIdStartAt = SequentailIdStartAt;
}
parameter_types! {
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		AssetRegistry: pallet_asset_registry::{Pallet, Call, Storage, Event<T>, Config<T>} = 42,
			XTokens: orml_xtokens::{Pallet, Call, Storage, Event<T>, } = 43,
			TechnicalCommittee: pallet_collective::<Instance2> = 45,
			Tokens: orml_tokens::{Pallet, Call, Storage, Event<T>, } = 44,
			UnknownTokens :orml_unknown_tokens = 46,
			Currencies : orml_currencies = 47,
			OrmlXCM : orml_xcm = 48,
			Council: pallet_collective::<Instance1> = 49,
	}
);
